...
/*   Updated: 2023/12/10 08:37:47 by olahrizi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void get_distance2(t_ray *back_ray, t_vec3 *vhat, float *t, t_vec3 *poi, float t2)
{
	if (t2 == FLT_MAX)
	{
		t[0] = 100e6;
...
int test_cylinder(t_ray *ray, t_info *info)
{
	...
	min_index = 0;
	float min = get_min(t, &min_index);
	if (min == -1)
		return (0);
	validpoi = poi[min_index];
	return(calculate_props(min_index, &validpoi, info, &vhat));
}