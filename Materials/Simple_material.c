...
t_vec3 compute_color(t_vars *vars, t_info *info, t_ray *camera_ray, int ref_count)
{
	...
	{
		diff_color = diffuse_color(vars->objects, info, vars->lights, get_checker_color(info, info->e->checker_matrix));
	}
	else if (info->e->has_texture == 2)
	{
		diff_color = diffuse_color(vars->objects, info, vars->lights, get_color_texture(info->e->image, info->uv.u, info->uv.v));
	}
	else
		diff_color = diffuse_color(vars->objects, info, vars->lights, info->e->base_color);
	mat_color = diff_color;
	if (info->e->has_material)
	{	
		...
		if (info->e->shininess)
			spc_color = specular_highlight(vars->objects, info, vars->lights, camera_ray);
		mat_color = vec_add(mat_color, spc_color);
	}
	t_vec3 ambient = scale_vector(new_vector(0.3, 0.3, 0.3), 0.1);

	return (vec_add(mat_color, ambient));
}