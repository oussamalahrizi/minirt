...
t_vec3 compute_color(t_vars *vars, t_info *info, t_ray *camera_ray, int ref_count)
{
	...
	{
		diff_color = diffuse_color(info, vars, get_checker_color(info, info->e->checker_matrix));
	}
	else if (info->e->has_texture == 2)
	{
		diff_color = diffuse_color(info, vars, get_color_texture(info->e->image, info->uv.u, info->uv.v));
	}
	else
		diff_color = diffuse_color(info, vars, info->e->base_color);
