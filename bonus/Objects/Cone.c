...
/*   Updated: 2024/01/04 02:00:43 by olahrizi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_t2(t_vec3 *ray_org, t_vec3 *vhat, float *t, t_vec3 *poi)
{
	...
	b = 2 * (ray_org->x * vhat->x + ray_org->y * vhat->y
			- ray_org->z * vhat->z);
...
static float	get_distance(t_ray *back_ray, t_vec3 *vhat,
	float *t, t_vec3 *poi)
{
	int	t2;
...
int	test_cone(t_ray *ray, t_info *info)
{
	...
	min_index = 0;
	if (get_min(t, &min_index, 3) == FLT_MAX)
		return (0);
	return (calculate_cone_props(min_index, &poi[min_index], info, &vhat));
}
